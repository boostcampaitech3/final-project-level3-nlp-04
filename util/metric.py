...
y_true = {"이름": "박원주",
          "직책": "금융부문이사",
          "회사명": "차투차",
          "주소": "경기도 수원시 권선구 세화로 44 (평동)",
          "전화번호":	"-",
          "이메일": "wjp3020@gmail.com"}
...
def calculate_metric(y_true: Dict[str, str], y_pred: Dict[str, str], threshold: int = 0.1) -> Dict[str, int]:

    y_true_value = [x for x in y_true.values()]
    y_pred_value = [x for x in y_pred.values()]

    y_true_bin = [1 if x != '-' else 0 for x in y_true_value]
    y_pred_bin = []
...
def calculate_accuracy(true_list: List[Dict[str, str]], pred_list: List[Dict[str, str]], threshold: int = 0.8) -> int:

    cnt = 0
    for y_true, y_pred in zip(true_list, pred_list):
        if calculate_metric(y_true, y_pred)['f1_score'] >= threshold:
            cnt += 1
